...
#include <math.h>  // Library for sin and cosine functions
#include <std_msgs/UInt8.h> // Standard int message type for ros
...
int iPubFlag=0;  // Flag to publish curren position
i90_position::pos currentPos; // Current i90 position
i90_position::pos prevPos;    // i90 position for previous state
...
int main(int argc, char **argv){
	...
	prevPos.fXPos=0;   // Initializing previous position to origin and yaw angle to 45 degrees
	prevPos.fYPos=0;
	prevPos.fYawAngle=45.00;
	
	while (ros::ok()){
		// d.sleep();
		if(iPubFlag==1){  // Publish current position only when publish flag is 1
...
void recalculateTranslation(const std_msgs::UInt8 iTransFlag){
	...
	iCurEnc[1]=32768-iCurEnc[1]; // Converts the encoder values to match with left wheel.
	iEncoderDifference[0]=iCurEnc[0]-iPrevEnc[0];   // Difference in encoder values of current and previous readings
	...
	iPubFlag=1;  //sets publish flag 
	
	

	
	ROS_INFO("New position after translational motion: [%f]/t[%f]/t", currentPos.fXPos, currentPos.fYPos,currentPos.fYawAngle);
}
...
ROS_INFO("New position after rotational motion: [%f]/t[%f]/t", currentPos.fXPos, currentPos.fYPos,currentPos.fYawAngle);
}

void updateEncoder(const drrobot_I90_player::MotorInfoArray motorInfoArray){
	
	iCurEnc[0] = motorInfoArray.motorInfos[0].encoder_pos;  // Reads the encoder values and store in the local variable
	iCurEnc[1] = motorInfoArray.motorInfos[1].encoder_pos;
	

	ROS_INFO("New encoder readings:left wheel: [%d]/tRight wheel: [%d]", iCurEnc[0],iCurEnc[1]);
}
